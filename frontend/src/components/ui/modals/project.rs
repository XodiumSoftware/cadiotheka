...
const WIREFRAME_KEY: &str = "wireframe";
...
/// Default selection outline color used when no viewer preferences are saved.
const DEFAULT_OBJECT_SELECTION_COLOR: Srgba = Srgba::new(0, 150, 255, 255);
...
/// Loads the saved field of view from account viewer preferences.
fn load_fov_degrees_from_preferences(account: Option<&AccountData>) -> f32 {
    let Some(account) = account else {
...
/// Returns the account's existing viewer preferences with the field of view
/// updated, preserving any other keys that may exist in the JSON blob.
fn preferences_with_fov_degrees(account: Option<&AccountData>, degrees: f32) -> Option<String> {
    let account = account?;
    let mut prefs: serde_json::Value =
        serde_json::from_str(&account.viewer_preferences).unwrap_or(serde_json::json!({}));
...
/// Loads the saved wireframe rendering setting from account viewer preferences.
fn load_wireframe_from_preferences(account: Option<&AccountData>) -> bool {
    let Some(account) = account else {
        return false;
    };
    let prefs: serde_json::Value =
...
/// Returns the account's existing viewer preferences with the wireframe rendering
/// setting updated, preserving any other keys that may exist in the JSON blob.
fn preferences_with_wireframe(account: Option<&AccountData>, wireframe: bool) -> Option<String> {
    let account = account?;
    let mut prefs: serde_json::Value =
        serde_json::from_str(&account.viewer_preferences).unwrap_or(serde_json::json!({}));
    prefs[WIREFRAME_KEY] = serde_json::json!(wireframe);
    serde_json::to_string(&prefs).ok()
}
...
/// Loads the saved object selection color from account viewer preferences.
fn load_object_selection_color_from_preferences(account: Option<&AccountData>) -> Srgba {
    let Some(account) = account else {
        return DEFAULT_OBJECT_SELECTION_COLOR;
    };
    let prefs: serde_json::Value =
        serde_json::from_str(&account.viewer_preferences).unwrap_or(serde_json::json!({}));
    prefs
        .get(OBJECT_SELECTION_COLOR_KEY)
        .and_then(serde_json::Value::as_str)
        .and_then(hex_to_srgba)
        .unwrap_or(DEFAULT_OBJECT_SELECTION_COLOR)
}

/// Returns the account's existing viewer preferences with the object selection color
/// updated, preserving any other keys that may exist in the JSON blob.
fn preferences_with_object_selection_color(
    account: Option<&AccountData>,
    color: Srgba,
) -> Option<String> {
    let account = account?;
    let mut prefs: serde_json::Value =
        serde_json::from_str(&account.viewer_preferences).unwrap_or(serde_json::json!({}));
    prefs[OBJECT_SELECTION_COLOR_KEY] = serde_json::json!(srgba_to_hex(color));
    serde_json::to_string(&prefs).ok()
}
...
fn ProjectModalContent(
    ...
    &visible_save_generation);
                                            ...
                                            let fov_degrees = RwSignal::new(load_fov_degrees_from_preferences(
                                                current_user.account.get_untracked().as_ref(),
                                            ));
                                            ...
                                            Effect::new({
                                                let current_user = current_user;
                                                let set_fov_degrees = fov_degrees;
                                                move |_| {
                                                    let Some(account) = current_user.account.get() else {
                                                        return;
                                                    };
                                                    let new_fov = load_fov_degrees_from_preferences(Some(&account));
                                                    if (set_fov_degrees.get_untracked() - new_fov).abs() > f32::EPSILON {
                                                        set_fov_degrees.set(new_fov);
                                                    }
                                                }
                                            });
                                            ...
                                            let initial_fov_degrees = fov_degrees.get_untracked();
                                            let fov_save_generation = Rc::new(std::cell::Cell::new(0u64));

                                            Effect::new({
                                                let current_user = current_user;
                                                let profile_modal = profile_modal;
                                                let fov_save_generation = Rc::clone(&fov_save_generation);
                                                move |_| {
                                                    let degrees = fov_degrees.get();
                                                    if (degrees - initial_fov_degrees).abs() <= f32::EPSILON {
                                                        return;
                                                    }
                                                    let Some(account) =
                                                        current_user.account.get_untracked()
                                                    ...
                                                    let expected = fov_save_generation
                                                        .get()
                                                        .wrapping_add(1);
                                                    fov_save_generation.set(expected);

                                                    let set_current_user = current_user.set_account;
                                                    let set_profile_account = profile_modal.set_account;
                                                    let fov_save_generation = Rc::clone(
                                                        &fov_save_generation);
                                                    leptos::task::spawn_local(async move {
                                                        gloo_timers::future::TimeoutFuture::new(300)
                                                            .await;
                                                        if fov_save_generation.get() != expected {
                                                            return;
                                                        }
                                            ...
                                            let wireframe = RwSignal::new(load_wireframe_from_preferences(
                                                current_user.account.get_untracked().as_ref(),
                                            ));

                                            Effect::new({
                                                let current_user = current_user;
                                                let set_wireframe = wireframe;
                                                move |_| {
                                                    let Some(account) = current_user.account.get() else {
                                                        return;
                                                    };
                                                    let new_wireframe =
                                                        load_wireframe_from_preferences(Some(&account));
                                                    if set_wireframe.get_untracked() != new_wireframe {
                                                        set_wireframe.set(new_wireframe);
                                                    }
                                                }
                                            });

                                            let initial_wireframe = wireframe.get_untracked();
                                            let wireframe_save_generation = Rc::new(std::cell::Cell::new(0u64));

                                            Effect::new({
                                                let current_user = current_user;
                                                let profile_modal = profile_modal;
                                                let wireframe_save_generation = Rc::clone(&wireframe_save_generation);
                                                move |_| {
                                                    let wireframe_value = wireframe.get();
                                                    if wireframe_value == initial_wireframe {
                                                        return;
                                                    }
                                                    let Some(account) =
                                                        current_user.account.get_untracked()
                                                    ...
                                                    let expected = wireframe_save_generation
                                                        .get()
                                                        .wrapping_add(1);
                                                    wireframe_save_generation.set(expected);

                                                    let set_current_user = current_user.set_account;
                                                    let set_profile_account = profile_modal.set_account;
                                                    let wireframe_save_generation = Rc::clone(
                                                        &wireframe_save_generation);
                                                    leptos::task::spawn_local(async move {
                                                        gloo_timers::future::TimeoutFuture::new(300)
                                                            .await;
                                                        if wireframe_save_generation.get() != expected {
                                                            return;
                                                        }
                                            ...
                                            let selection_color = RwSignal::new(
                                                load_object_selection_color_from_preferences(
                                                    current_user.account.get_untracked().as_ref(),
                                                ),
                                            );
                                            ...
                                            Effect::new({
                                                let current_user = current_user;
                                                let set_selection_color = selection_color;
                                                move |_| {
                                                    let Some(account) = current_user.account.get() else {
                                                        return;
                                                    };
                                                    ...
                                                    if set_selection_color.get_untracked() != new_color {
                                                        set_selection_color.set(new_color);
                                                    }
                                                }
                                            });

                                            let initial_selection_color = selection_color.get_untracked();
                                            let selection_save_generation =
                                                Rc::new(std::cell::Cell::new(0u64));

                                            Effect::new({
                                                let current_user = current_user;
                                                let profile_modal = profile_modal;
                                                let selection_save_generation = Rc::clone(
                                                    &selection_save_generation);
                                                move |_| {
                                                    let color = selection_color.get();
                                                    if color == initial_selection_color {
                                                        return;
                                                    }
                                                    let Some(account) =
                                                        current_user.account.get_untracked()
                                                    ...
                                                    let Some(new_preferences) =
                                                        preferences_with_object_selection_color(
                                                            Some(&account),
                                                            color,
                                                        )
                                                    else {
                                                    ...
                                                    let expected = selection_save_generation
                                                        .get()
                                                        .wrapping_add(1);
                                                    selection_save_generation.set(expected);
                                                    ...
                                                    leptos::task::spawn_local(async move {
                                                        gloo_timers::future::TimeoutFuture::new(300)
                                                            .await;
                                                        if selection_save_generation.get() != expected {
                                                            return;
                                                        }
                                            ...
                                            let fps_value: RwSignal<u32> = RwSignal::new(0);
                                        ...
                                        <div node_ref=viewer_ref class="h-full flex flex-col">
                                            <div class="flex items-center justify-between gap-2 rounded-none border border-base-content/10 bg-base-200/30 p-2 flex-shrink-0">
                                                <span class="text-xs font-mono text-base-content/70 px-1">
                                                    {move || if show_fps.get() { format!("{} FPS", fps_value.get()) } else { String::new() }}
                                                </span>
